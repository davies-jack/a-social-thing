    ...
    <main>
      <div className="bg-bg-secondary p-6 rounded-md">
        ...
        <div className="mt-1 flex flex-row gap-2 text-sm">
          <span className="hover:text-headline-text hover:cursor-pointer">{amountOfFollowers} followers</span>
          <span className="hover:text-headline-text hover:cursor-pointer">{amountOfFollowing} following</span>
        </div>

        {!isOurProfile && (
          <form action={handleFollowUser}>
            <button className={
              `text-button-text p-2 px-4 rounded-md text-sm mt-4 font-bold
              ${following ? "bg-bg-button-secondary border border-bg-button hover:bg-bg-button transition-colors duration-75" : "bg-bg-button hover:bg-bg-button-secondary shadow-md border"}
              `
      ...
      <ul className="rounded-md m-6 text-sm text-headline-text py-1">
        {posts.map((post, index) => {
          ...
          return (
            <li key={post.id} className={`bg-bg-secondary p-6 ${isFirst ? 'mt-0' : 'mt-4'} rounded-md`}>
              <span>{post.status}</span>
              <span className="block font-bold text-paragraph-text text-xs mt-2">{formatDate(post.createdAt)}</span>
            </li>
          );
        })}
      </ul>
